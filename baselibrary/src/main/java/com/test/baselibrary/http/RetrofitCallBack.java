...
import com.test.baselibrary.Utils.StringUtills;
import com.test.baselibrary.module.ActivityManager;
import com.test.baselibrary.http.httpResponse.BaseResponse;
...
public abstract class RetrofitCallBack<T> implements Callback<T> {
    // 日志标识
    private final static String TAG = "http@RetrofitCallBack";
    // 页面弱引用为空
    ...
    public RetrofitCallBack(ApiCallback<T> mCallback, Activity activity,
    ...
    public void onResponse(Call<T> call, Response<T> response) {
        ...
        if (!ActivityManager.getInstance().isCurrent(activityWeakRef.get())){
        ...
        // 无数据回调处理
        if (StringUtills.isEmpty(body)){
            mCallback.onError(NULL_DATA);
            Log.e(TAG, "#" + NULL_DATA);
            return;
        }
        if (StringUtills.isNotEmpty(requestId)){
            switch (requestId){
                case REQUEST_ID_ONE:
                    // 数据格式一处理
    ...
    private void requestIdTwoDeal(String body) {
        if (StringUtills.isNotEmpty(body)){
    ...
    private void requestIdOneDeal(String body) {
        try {
            ...
            // 请求成功
            if (STATUS_IDENTIFY_ZORE.equals(errorCode)
                    && StringUtills.isNotEmpty(result)){
                // Gson解析数据，回调数据
                BaseResponse baseResponse;
