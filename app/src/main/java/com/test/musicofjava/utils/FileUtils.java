package com.test.musicofjava.utils;
...
public class FileUtils {
    ...
    public static String getLrcFilePath(Music music) {
        ...
        if (!exists(lrcFilePath)) {
            lrcFilePath = changeFileName(music.getArtist(), music.getTitle());
     ...
     * @return
     */
    private static String changeFileName(String artist, String title) {
        String[] lrcArtists = null;
        lrcArtists = artist.split("\\,");

        StringBuffer sb = new StringBuffer();
        for (int i = lrcArtists.length - 1; i > -1; i--) {
            sb.append(lrcArtists[i]).append(",");
        }
        if (sb.length() != 0) {
            sb.deleteCharAt(sb.length() - 1);
        }
        return getLrcDir() + getLrcFileName(String.valueOf(sb), title);
    }

    public static String getLrcPath(Music music) {
        if (music == null) {
            return null;
        }

        String lrcFilePath = Environment.getExternalStorageDirectory().getPath();
        if (!exists(lrcFilePath)) {
            lrcFilePath = music.getPath().replace(MP3, LRC);
            if (!exists(lrcFilePath)) {
                lrcFilePath = null;
